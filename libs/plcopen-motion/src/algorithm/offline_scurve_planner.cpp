...
{
  // Deceleration period
  if ((v_max - v) * j_max < sq(a_max))  // (3.19)(3.20)
  {
    ...
    const ScurveCondition& condition, ScurveProfile& profile)
{
  ...
  double v = sq(a_max) / j_max;
  double delta =
      sq(a_max) * sq(a_max) / sq(j_max) +
      2 * (sq(v0) + sq(v1)) +
    ...
    double t, const ScurveProfile& profile, const ScurveCondition& condition)
{
  ...
  auto trajectory = [&](double t) -> double* {
    ...
    else if (tj1 <= t && t < (ta - tj1))
    {
      // (3.30b)
      q = q0 + v0 * t +
          a_lim_a * (3 * sq(t) - 3 * tj1 * t + sq(tj1)) / 6;
    ...
    else if ((ta - tj1) <= t && t < ta)
    {
      tt = ta - t;
      // (3.30c)
      q = q0 + (v_lim + v0) * ta / 2 - v_lim * tt - j_min * cb(tt) / 6;
      v = v_lim + j_min * sq(tt) / 2;
      a = -j_min * tt;
      j = j_min = -j_max;
    }
    ...
    else if ((tw - td) <= t && t < (tw - td + tj2))
    {
      tt = t - tw + td;
      // (3.30e)
      q = q1 - (v_lim + v1) * td / 2 + v_lim * tt - j_max * cb(tt) / 6;
      v = v_lim - j_max * sq(tt) / 2;
      a = -j_max * tt;
      j = j_min = -j_max;
    }
    else if ((tw - td + tj2) <= t && t < (tw - tj2))
    {
      ...
      q = q1 - (v_lim + v1) * td / 2 + v_lim * tt +
          a_lim_d * (3 * sq(tt) - 3 * tj2 * tt + sq(tj2)) / 6;
