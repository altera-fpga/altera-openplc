                                                     ...
                                                     double& Td)
{
  ...
  // Equation (6)
  hk = ak_ * sq(Td) / 2.0 +
       (jmin * Tj2a * (3.0 * sq(Td) - 3.0 * Td * Tj2a + sq(Tj2a)) +
        jmax * cb(Tj2b)) /
           6.0 +
       Td * vk_;
}
                                                     ...
                                                     double& Td)
{
  ...
  // Equation (10)
  hk = ak_ * sq(Td) / 2.0 +
       (jmax * Tj2a * (3.0 * sq(Td) - 3.0 * Td * Tj2a + sq(Tj2a)) +
        jmin * cb(Tj2b)) /
           6.0 +
       Td * vk_;
}
                                              ...
                                              double& vk, double& sk)
{
  ...
  if (phase_ != mcJerkMin)
  {
    ...
    a3      = (20 * h - (8 * condition_.v1 + 12 * v_init_) * td -
          (3 * a_init_ - condition_.a1) * sq(td)) /
         (2 * cb(td));
    a4 = (-30 * h + (14 * condition_.v1 + 16 * v_init_) * td +
          (3 * a_init_ - 2 * condition_.a1) * sq(td)) /
         (2 * sq(td) * sq(td));
    a5 = (12 * h - 6 * (condition_.v1 + v_init_) * td +
          (condition_.a1 - a_init_) * sq(td)) /
         (2 * sq(td) * cb(td));
  ...
  {
    double tt = t - t_init_;
    sk        = s_init_ + a1 * tt + a2 * sq(tt) + a3 * cb(tt) +
         a4 * sq(tt) * sq(tt) + a5 * sq(tt) * cb(tt);
    vk = v_init_ + 2 * a2 * tt + 3 * a3 * sq(tt) + 4 * a4 * cb(tt) +
         5 * a5 * sq(tt) * sq(tt);
    ak = a_init_ + 6 * a3 * tt + 12 * a4 * sq(tt) + 20 * a5 * cb(tt);
    jk = 6 * a3 + 24 * a4 * tt + 60 * a5 * sq(tt);
  }
#else
  // Equation (7)
  if (t - th < Tj2a)
  {
    ...
    jk = Jmin;
    ak = a_init_ - Jmax * tt;
    vk = v_init_ + a_init_ * tt - Jmax * sq(tt) / 2.0;
    sk = s_init_ + v_init_ * tt + a_init_ * sq(tt) / 2.0 -
         Jmax * cb(tt) / 6.0;
  }
                                              ...
                                              double& vk, double& sk)
{
  ...
  if ((t - th) > (Td - Tj2b) && sk_ < condition_.q1)
  {
    ...
    jk = Jmin;
    ak = a_init_ - Jmax * tt;
    vk = v_init_ + a_init_ * tt - Jmax * sq(tt) / 2;
    sk = s_init_ + v_init_ * tt + a_init_ * sq(tt) / 2.0 -
         Jmax * cb(tt) / 6.0;
  }
#endif
                                               ...
                                               double& sk)  // v0 <= Vmax
{
  ...
  double tmp = vk_ - sq(ak_) / (2.0 * jmin);
  // Equation (2)
  ...
  if (tmp < vmax && ak_ >= amax)  // Accelerate constant
  {
    ...
    ak = amax;
    vk = v_init_ + amax * tt;
    sk = s_init_ + v_init_ * tt + amax * sq(tt) / 2.0;
  }
                                               ...
                                               double& sk)  // v0 > Vmax
{
  ...
  double tmp = vk_ - sq(ak_) / (2.0 * jmax);
  // Equation (3)
  // double jk = 0.0;
  ...
  if (tmp > vmax && ak_ <= amin)  // Accelerate constant
  {
    ...
    ak = amin;
    vk = v_init_ + amin * tt;
    sk = s_init_ + v_init_ * tt + amin * sq(tt) / 2.0;
  }
