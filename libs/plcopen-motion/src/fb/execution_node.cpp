...
ExecutionNode& ExecutionNode::operator=(const ExecutionNode& node)
{
  if (this != &node)
  {
    ...
    pos_tmp_         = node.pos_cmd_;
    vel_tmp_         = node.vel_cmd_;
    acc_tmp_         = node.acc_cmd_;
...
void ExecutionNode::onExecution(mcLREAL master_ref_pos, mcLREAL master_ref_vel)
{
  ...
  if (need_plan_ == mcTRUE)
  {
    ...
    planner_.setCondition(start_pos_, end_pos_, start_vel_, end_vel_,
                          start_acc_, end_acc_, duration_, velocity_,
                          acceleration_, jerk_, planner_type_);
  ...
  switch (motion_mode_)
  {
    ...
    case mcMoveRelativeMode: {
      if (fabs(axis_pos - end_pos_) <= pos_done_factor_ &&
          fabs(axis_vel - end_vel_) <= vel_done_factor_)
        res = mcTRUE;
    ...
    case mcGearInMode:
    case mcMoveVelocityMode: {
      if (fabs(axis_vel - end_vel_) <= vel_done_factor_)
        res = mcTRUE;
    }
    break;
    case mcHomingMode: {
