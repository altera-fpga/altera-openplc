...
void Axis::runCycle()
{
  ...
  if (statusHealthy() == mcTRUE)
  {
    ...
    motion_kernel_.runCycle(master_ref_pos_, master_ref_vel_, axis_pos_,
                            axis_vel_cmd_, &axis_state_);
...
void Axis::syncMotionKernelResultsToAxis()
{
      ...
      (axis_mode_ == mcServoControlModeHomeServo))
  {
    ...
    motion_kernel_.getCommands(&axis_pos_cmd_, &axis_vel_cmd_, &axis_acc_cmd_);
